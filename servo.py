...
# Konfigurasi logging untuk debugging dan pelacakan error
logging.basicConfig(
...
# URL untuk ESP32-CAM dan servo
CAMERA_URL = "http://192.168.1.12:81/stream"  # URL kamera ESP32-CAM
BASE_URL = "http://192.168.1.11:8000"  # IP untuk kontrol servo
...
async def send_telegram_photo(photo, caption):
    ...
    Args:
        photo (bytes): Data foto dalam format bytes.
...
# --- FUNGSI PENGAMBILAN DATA UBIDOTS ---
def get_ubidots_data(variable_label):
    """
    Mengambil data dari Ubidots untuk variabel tertentu.
    
    Args:
        variable_label (str): Nama variabel (misalnya, 'mq2', 'humidity').
    
    Returns:
        list: Daftar data dari Ubidots, atau None jika gagal.
    """
...
# --- FUNGSI PENGAMBILAN DATA SENSOR TERBARU ---
def fetch_latest_sensor_data():
    """
    Mengambil data terbaru dari Ubidots untuk semua variabel sensor.
    
    Returns:
        dict: Dictionary berisi nilai dan status untuk setiap sensor.
    """
    data_values = {}
    statuses = {}
    ...
    statuses["mq2"] = predict_smoke_status(data_values.get("mq2")) if data_values.get("mq2") is not None else "Data asap tidak tersedia"
    statuses["lux"] = evaluate_lux_condition(data_values.get("lux"), data_values.get("mq2")) if data_values.get("lux") is not None else "Data cahaya tidak tersedia"
    statuses["temperature"] = evaluate_temperature_condition(data_values.get("temperature")) if data_values.get("temperature") is not None else "Data suhu tidak tersedia"
    statuses["humidity"] = (
    ...
    statuses["mic"] = evaluate_mic_condition(data_values.get("mic")) if data_values.get("mic") is not None else "Data amplitudo tidak tersedia"
...
def generate_mq2_simulation_data(n_samples=100):
    """
    Menghasilkan data simulasi untuk sensor MQ2.
    
    Args:
        n_samples (int): Jumlah sampel data.
    
    Returns:
        pd.DataFrame: DataFrame dengan nilai MQ2 dan label.
    """
    data = []
...
def train_mq2_model():
    """
    Melatih model LogisticRegression untuk prediksi asap.
    
    Returns:
        LogisticRegression: Model yang telah dilatih.
    """
...
def predict_smoke_status(mq2_value):
    """
    Memprediksi status asap berdasarkan nilai MQ2.
    
    Args:
        mq2_value (float): Nilai sensor MQ2.
    
    Returns:
        str: Status asap (Bahaya, Mencurigakan, atau Aman).
    """
...
def evaluate_lux_condition(lux_value, mq2_value):
    """
    Mengevaluasi kondisi pencahayaan berdasarkan nilai lux dan MQ2.
    
    Args:
        lux_value (float): Nilai sensor lux.
        mq2_value (float): Nilai sensor MQ2.
    
    Returns:
        str: Status pencahayaan.
    """
...
def evaluate_temperature_condition(temp_value):
    """
    Mengevaluasi kondisi suhu.
    
    Args:
        temp_value (float): Nilai suhu dalam Celsius.
...
def evaluate_mic_condition(mic_value):
    """
    Mengevaluasi kondisi amplitudo suara.
    
    Args:
        mic_value (float): Nilai sensor mikrofon.
    
    Returns:
        str: Status amplitudo.
    """
...
def generate_narrative_report(mq2_status, mq2_value, lux_status, lux_value, temp_status, temp_value, humidity_status, humidity_value, mic_status, mic_value):
    """
    Menghasilkan laporan naratif berdasarkan data sensor.
    
    Args:
        mq2_status (str): Status asap.
        mq2_value (float): Nilai MQ2.
        lux_status (str): Status cahaya.
        lux_value (float): Nilai lux.
        temp_status (str): Status suhu.
        temp_value (float): Nilai suhu.
        humidity_status (str): Status kelembapan.
        humidity_value (float): Nilai kelembapan.
        mic_status (str): Status amplitudo.
        mic_value (float): Nilai amplitudo.
...
def predict_smoking_risk_rule_based(mq2_value, lux_value, mic_value):
    """
    Memprediksi risiko merokok berdasarkan aturan.
...
def get_room_condition_summary(mq2_value, lux_value, temperature_value, humidity_value, mic_value):
    """
    Menghasilkan ringkasan kondisi ruangan.
    
    Args:
        mq2_value (float): Nilai MQ2.
        lux_value (float): Nilai lux.
        temperature_value (float): Nilai suhu.
        humidity_value (float): Nilai kelembapan.
        mic_value (float): Nilai amplitudo.
    
    Returns:
        dict: Ringkasan status, warna, saran, dan detail.
    """
...
def get_gemini_response(messages):
    ...
    Returns:
        str: Respons dari AI atau pesan error.
    """
    headers = {
...
def generate_chatbot_context(mq2_value, lux_value, temperature_value, humidity_value, mic_value):
    """
    Menghasilkan konteks untuk chatbot berdasarkan data sensor.
...
def load_yolo_model():
    """
    Memuat model YOLOv5 untuk deteksi objek.
    
    Returns:
        torch.nn.Module: Model YOLOv5, atau None jika gagal.
    """
    try:
...
async def run_camera_detection(frame_placeholder, status_placeholder):
    """
    Menjalankan deteksi kamera menggunakan ESP32-CAM dan YOLOv5.
    
    Args:
        frame_placeholder: Placeholder untuk menampilkan frame kamera.
        status_placeholder: Placeholder untuk menampilkan status deteksi.
    """
    try:
...
# --- FUNGSI NOTIFIKASI PERIODIK ---
async def send_periodic_notification():
    """
    Mengirim notifikasi periodik ke Telegram setiap 5 menit dengan data sensor.
    """
    current_time = time.time()
...
def run_async(coro):
    """
    Menjalankan fungsi asinkronus secara sinkronus.
    
    Args:
        coro: Coroutine yang akan dijalankan.
    
    Returns:
        Hasil dari coroutine, atau False jika gagal.
    """
    try:
...
# --- KONTROL SERVO ---
def control_servo(sudut):
    """
    Mengirim perintah sudut ke servo melalui ESP32.
    
    Args:
        sudut (int): Sudut servo (0-180).
    
    Returns:
        bool: True jika berhasil, False jika gagal.
    """
    try:
        response = requests.post(f"{BASE_URL}/arah", json={"sudut": sudut}, timeout=5)
        if response.ok:
            logger.info(f"Sudut {sudut} berhasil dikirim ke ESP32")
            return True
        else:
            logger.error(f"Gagal mengirim sudut: {response.status_code}")
            return False
    except requests.exceptions.RequestException as e:
        logger.error(f"Error saat mengirim sudut: {str(e)}")
        return False
...
def main():
    ...
    with tab1:
        ...
        # Tampilkan ringkasan kondisi ruangan
    ...
    with tab2:
        ...
        # Kontrol Servo
        st.subheader("Kontrol Servo Sudut Bebas")
        st.write("Gunakan slider di bawah ini untuk mengatur sudut servo yang menggerakkan kamera ESP32-CAM.")
        if "prev_sudut" not in st.session_state:
            st.session_state.prev_sudut = 90
        sudut = st.slider(
            "Pilih sudut servo (0-180)",
            min_value=0,
            max_value=180,
            value=90,
            step=1,
        ...
        if sudut != st.session_state.prev_sudut:
            success = control_servo(sudut)
            if success:
                st.success(f"Sudut {sudut}° berhasil dikirim ke ESP32")
                st.session_state.prev_sudut = sudut
            else:
                st.error(f"Gagal mengirim sudut {sudut}° ke ESP32")

        # Placeholder untuk kamera dan status
